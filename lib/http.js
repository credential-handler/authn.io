...
bedrock.events.on('bedrock-express.configure.routes', app => {
  ...
  app.get('/manifest', asyncHandler(async (req, res, next) => {
    // Note: `origin` is really `authority` as in `host[:port]`
    const {origin} = req.query;
    if(!origin) {
      throw new BedrockError(
        'An "origin" query parameter must be given.',
...
async function getResponse({origin}) {
  ...
  const manifestUrl = `https://${origin}${manifestPath}`;
