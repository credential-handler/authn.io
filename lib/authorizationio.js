...
bedrock.events.on('bedrock-mongodb.ready', function(callback) {
  ...
  async.waterfall([
    ...
    database.collections.didDocument.save({
      '@context': 'https://w3id.org/identity/v1',
      id: 'did:d1d1d1d1-d1d1-d1d1-d1d1-d1d1d1d1d1d1',
      credentialsRequestUrl: 'https://credentials.dev:33443/credentials/',
      accessControl: {
        writePermission: [{
    ...
    }, database.writeOptions,function(err, doc) {
      if(err) {
  ...
  */
  app.post('/credentials-request', function(req, res, next) {
    ...
    views.getDefaultViewVars(req, function(err, vars) {
      ...
      if(request) {
        if(request.callback) {
          vars.callback = request.callback;
        }
        if(request.credential) {
          vars.credential = request.credential;
        }
      }
      res.render('index.html', vars);
    });
